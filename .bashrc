...
echo ansible-vault argon2 yq | contains_element ${cmd_avail[@]} > /dev/null 2>&1 && {
    ...
    vault > /dev/null 2>&1 && {
        ...
        install -m 400 <(yq -r --arg USER ${USER:-$(whoami)} '.[$USER]["private_key_content"]' <(vault)) "${TMPDIR}/id_rsa"
        ...
        add password    *       $(printf "%q\t%q" $(yq -r --arg USER ${USER:-$(whoami)} '.[$USER]|[.tacacs,.tacacs]|@tsv' <(vault)))
